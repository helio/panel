...
class CookieHelperTest extends TestCase
{
    ...
    public function testAddCookie(): void
    {
        ...
        $response = CookieHelper::addCookie($response, 'test', 'value');
    ...
    public function testDeleteCookie(): void
    {
        ...
        $response = CookieHelper::deleteCookie($response, 'test')->getHeader('set-cookie');
    ...
    public function testExpiresInAddCookie(): void
    {
        ...
        $now = new \DateTimeImmutable('now', new \DateTimeZone(CookieHelper::$timeZone));
        $future = new \DateTimeImmutable('now +2 weeks', new \DateTimeZone(CookieHelper::$timeZone));

        $responseCookies = CookieHelper::addCookie($emptyResponse, 'test', 'value',
    ...
    protected function getExpires(string $cookieValue): \DateTimeInterface
    {
        ...
        return new \DateTimeImmutable($match[1], new \DateTimeZone(CookieHelper::$timeZone));
    }
